...
# To ensure consistency in the language detected by detect_lang()
...
class DateExtractor(AbstractExtractor):
    ...
    def detect_lang(self, html):
        ...
        try:
            ...
            return detect(html.find('title').text)
    ...
    def _extract_from_json(self, html):
        ...
        try:
            ...
            scripts_three = [script for script in scripts_three if script is not None]
            for script in scripts_three:
                try:
                    if "post_date_gmt" in script.string:
                        return self.parse_date_str(script.string.split("post_date_gmt")[1])
                except:
                    pass
        except:
            return None
    ...
    def _extract_from_meta(self, html):
        ...
        try:
            for meta in html.find_all('meta'):
                ...
                '''
                Tested on:
                https://www.thehindu.com/news/national/kerala/orthodox-group-holds-mass-at-piravom-church/article29552580.ece
                '''
                if 'created-date' == meta_name:
                    date_from_meta = meta['content'].strip()
                    break
                ...
                if 'date.created' == meta_name:
                    date_from_meta = meta['content'].strip()
                    break

                if 'date.available' == meta_name:
                    date_from_meta = meta['content'].strip()
                    break
    ...
    # else:
        try:
            ...
            # Tested on https://timesofindia.indiatimes.com/business/india-business/parliamentary-panel-question-paytm-about-chinese-investment-google-over-resisting-data-localisation/articleshowprint/78942848.cms
