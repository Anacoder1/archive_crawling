...
Language isn't being extracted in the pipeline (as of now).
...
class ComparerLanguage:
    ...
    def extract(self, item, list_article_candidate):  # pylint: disable=too-many-branches,no-self-use,unused-argument
        """
        ...
        if max(languages_extracted_number)[0] == min(
                languages_extracted_number
        )[0] and language_newspaper is not None:
